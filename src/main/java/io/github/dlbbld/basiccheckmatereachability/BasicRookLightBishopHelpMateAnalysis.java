...
final class BasicRookLightBishopHelpMateAnalysis {
  ...
  private BasicRookLightBishopHelpMateAnalysis() {
  }
  ...
  static AnalysisResult analyze() {
        ...
        for (var blackKing = 0; blackKing < SQUARE_COUNT; blackKing++) {
          ...
          for (var blackBishop = 0; blackBishop < SQUARE_COUNT; blackBishop++) {
            ...
            for (var havingMove = WHITE_TO_MOVE; havingMove <= BLACK_TO_MOVE; havingMove++) {
              final var state = encode(whiteKing, whiteRook, blackKing, blackBishop, havingMove);
              ...
              final var blackInCheck = isBlackInCheck(whiteKing, whiteRook, blackKing, blackBishop);
              if (blackInCheck) {
                blackToMoveInCheckStateCount++;
                if (blackMoveMask(whiteKing, whiteRook, blackKing, blackBishop) == 0) {
                  blackCheckmateCount++;
                  winning.set(state);
                  queue.add(state);
                }
    ...
    for (var state = legalStates.nextSetBit(0); state >= 0; state = legalStates.nextSetBit(state + 1)) {
      ...
      if (havingMove(state) == WHITE_TO_MOVE) {
        ...
        final var blackBishop = blackBishop(state);
        if (!hasLegalWhiteMove(whiteKing, whiteRook, blackKing, blackBishop)) {
      ...
      final var blackKing = blackKing(state);
      final var blackBishop = blackBishop(state);
      final var blackMoveMask = blackMoveMask(whiteKing, whiteRook, blackKing, blackBishop);
      ...
      if (!hasLegalMove && !isBlackInCheck(whiteKing, whiteRook, blackKing, blackBishop)) {
        stalemateStateCount++;
  ...
  static String format(AnalysisResult result) {
    ...
    sb.append("KRvKB(light bishop) states").append('\n');
    ...
    sb.append("white-to-move states reducible by capturing the bishop: ")
  ...
  private static void appendStates(StringBuilder sb, String title, Set<RookLightBishopState> states) {
  ...
  private static void addWhitePredecessors(BitSet legalStates, BitSet winning, IntQueue queue, int state) {
    ...
    for (final int[] delta : KING_DELTAS) {
      ...
      if (origin != -1 && origin != whiteRook && origin != blackKing && origin != blackBishop) {
        addPredecessor(legalStates, winning, queue, origin, whiteRook, blackKing, blackBishop, WHITE_TO_MOVE);
      }
    }
    ...
    for (final int[] delta : ROOK_DELTAS) {
      ...
      while (origin != -1) {
        if (origin == whiteKing || origin == blackKing || origin == blackBishop) {
          break;
        }
        addPredecessor(legalStates, winning, queue, whiteKing, origin, blackKing, blackBishop, WHITE_TO_MOVE);
  ...
  private static void addBlackPredecessors(BitSet legalStates, BitSet winning, IntQueue queue, int state) {
    ...
    for (final int[] delta : KING_DELTAS) {
      ...
      if (origin != -1 && origin != whiteKing && origin != whiteRook && origin != blackBishop) {
        addPredecessor(legalStates, winning, queue, whiteKing, whiteRook, origin, blackBishop, BLACK_TO_MOVE);
      }
    }
  ...
  private static void addPredecessor(BitSet legalStates, BitSet winning, IntQueue queue, int whiteKing, int whiteRook,
      int blackKing, int blackBishop, int havingMove) {
    final var predecessor = encode(whiteKing, whiteRook, blackKing, blackBishop, havingMove);
  ...
  private static int blackMoveMask(int whiteKing, int whiteRook, int blackKing, int blackBishop) {
    var result = 0;
    for (final int[] delta : KING_DELTAS) {
  ...
  private static boolean hasWhiteCaptureToWinningRookEndgame(int whiteKing, int whiteRook, int blackKing,
      int blackBishop) {
    if (areKingsAdjacent(blackBishop, whiteKing) && isWhiteMoveLegal(blackBishop, whiteRook, blackKing, -1)) {
      return isRookEndgameKnownWinningForWhite(blackBishop, whiteRook, blackKing);
    }
  ...
  private static boolean hasLegalWhiteMove(int whiteKing, int whiteRook, int blackKing, int blackBishop) {
    for (final int[] delta : KING_DELTAS) {
      final var target = offset(whiteKing, delta);
      if (target == -1 || target == whiteRook || target == blackKing) {
        continue;
      }
      final var bishopAfterMove = target == blackBishop ? -1 : blackBishop;
      if (isWhiteMoveLegal(target, whiteRook, blackKing, bishopAfterMove)) {
        return true;
      }
    }
    ...
    for (final int[] delta : ROOK_DELTAS) {
      ...
      while (target != -1) {
        ...
        final var bishopAfterMove = target == blackBishop ? -1 : blackBishop;
        if (isWhiteMoveLegal(whiteKing, target, blackKing, bishopAfterMove)) {
          return true;
        }
        if (target == blackBishop) {
          break;
  ...
  private static boolean isWhiteMoveLegal(int whiteKing, int whiteRook, int blackKing, int blackBishop) {
    return !areKingsAdjacent(whiteKing, blackKing)
        && (blackBishop == -1 || !isBishopAttack(blackBishop, blackKing, whiteRook, whiteKing));
  }
  ...
  private static boolean isLegalState(int whiteKing, int whiteRook, int blackKing, int blackBishop, int havingMove) {
    return switch (havingMove) {
      case WHITE_TO_MOVE -> !isBlackInCheck(whiteKing, whiteRook, blackKing, blackBishop);
      case BLACK_TO_MOVE -> !isWhiteInCheck(whiteKing, whiteRook, blackKing, blackBishop);
      default -> throw new IllegalArgumentException();
    };
  }

  private static boolean isBlackInCheck(int whiteKing, int whiteRook, int blackKing, int blackBishop) {
    return areKingsAdjacent(whiteKing, blackKing) || isRookAttack(whiteRook, whiteKing, blackBishop, blackKing);
  }

  private static boolean isWhiteInCheck(int whiteKing, int whiteRook, int blackKing, int blackBishop) {
    return areKingsAdjacent(whiteKing, blackKing) || isBishopAttack(blackBishop, blackKing, whiteRook, whiteKing);
  }
  ...
  private static boolean isRookAttack(int whiteRook, int whiteKing, int blackBishop, int blackKing) {
    if (whiteRook == -1) {
      return false;
    }
    ...
    while (true) {
      ...
      if (square == whiteKing || square == blackBishop) {
        return false;
      }
      file += fileStep;
      rank += rankStep;
  ...
  private static boolean isBishopAttack(int blackBishop, int blackKing, int whiteRook, int whiteKing) {
    final var fileDistance = file(whiteKing) - file(blackBishop);
    final var rankDistance = rank(whiteKing) - rank(blackBishop);
    ...
    var file = file(blackBishop) + fileStep;
    var rank = rank(blackBishop) + rankStep;
    while (true) {
      final var square = square(file, rank);
      if (square == whiteKing) {
        return true;
      }
      if (square == blackKing || square == whiteRook) {
        return false;
      }
      file += fileStep;
      rank += rankStep;
  ...
  private static RookLightBishopState toState(int state) {
    return new RookLightBishopState(square(whiteKing(state)), square(whiteRook(state)), square(blackKing(state)),
        square(blackBishop(state)), side(havingMove(state)));
  }
  ...
  private static String toFen(RookLightBishopState state) {
    ...
    board[state.blackBishop().ordinal()] = 'b';
  ...
  private static String formatBlackMoves(RookLightBishopState state) {
    ...
    final var blackBishop = state.blackBishop().ordinal();
    final StringBuilder sb = new StringBuilder();

    appendLegalBlackKingMoves(sb, whiteKing, whiteRook, blackKing, blackBishop);
    appendLegalBlackBishopMoves(sb, whiteKing, whiteRook, blackKing, blackBishop);
    return sb.toString();
  }
  ...
  private static void appendLegalBlackBishopMoves(StringBuilder sb, int whiteKing, int whiteRook, int blackKing,
      int blackBishop) {
    for (final int[] delta : BISHOP_DELTAS) {
      var target = offset(blackBishop, delta);
      while (target != -1) {
        ...
        if (!isBlackInCheck(whiteKing, rookAfterMove, blackKing, target)) {
          appendMoveSeparator(sb);
          sb.append(square(blackBishop).getName()).append(square(target).getName());
          if (capturesRook) {
            sb.append("xR");
          }
        }
  ...
  record AnalysisResult(int legalStateCount, int whiteToMoveStateCount, int blackToMoveStateCount,
      ...
      Set<RookLightBishopState> stalemateRepresentatives, int counterexampleStateCount,
